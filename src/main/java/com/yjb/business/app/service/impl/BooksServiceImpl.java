...
public class BooksServiceImpl extends BaseService<BooksEntity> implements BooksService {
	...
	public String getBooksList() {
		Gson gson = new Gson();
		List<BooksEntity> list = booksMapper.selectBooksLimit(10);
	...
	public String insertBook(String title, String author) {
		Gson gson = new Gson();
		int cnt= booksMapper.insertBook( title,  author);
		String books="";
		if(cnt > 0) {
			books = gson.toJson("success").toString();
		} else {
			books = gson.toJson("failed").toString();
		}
